...
/*!
  ...
  additional couplings between the degrees of freedom (%DOF) in a FE grid.
...
public:
  ...
  /*!
    ...
    \details Each term consists of the %DOF index and an associated coefficient.
  */
  struct DOF
  {
    ...
    {
      int ivar = (dof.dof-1) / 3;
      int idof = (dof.dof+2) % 3;
      return s << (ivar < 1 ? 'u' : char('q'+ivar)) << '_'
               << char('x'+idof) <<" in node "<< dof.node;
    }
    ...
    int  dof;   //!< Local %DOF number within \ref node
    Real coeff; //!< The constrained value, or master %DOF scaling coefficient
    MPC* nextc; //!< Points to another MPC object in case of chained constraints
  };
