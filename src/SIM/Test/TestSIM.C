...
public:
  TestProjectSIM(const SIMinput::CharVec& nf) : Dim(nf)
  {
    ...
    REQUIRE(this->createDefaultModel());
    REQUIRE(this->preprocess());
  }
...
TEST_CASE("TestSIM2D.UniqueBoundaryNodes")
{
  ...
  SIM2D sim(1);
  REQUIRE(sim.loadXML(boundary_nodes));
...
TEST_CASE("TestSIM2D.Periodic")
{
  ...
  SIM2D sim(1);
  REQUIRE(sim.loadXML(geometry));
  REQUIRE(sim.createFEMmodel());
}


TEST_CASE("TestSIM2D.ProjectSolution")
{
  TestProjectSIM<SIM2D> sim({1});

  Matrix ssol;
  REQUIRE(sim.project(ssol, Vector(sim.getNoDOFs())));

  size_t n = 1;
  for (size_t j = 0; j < 2; ++j)
    for (size_t i = 0; i < 2; ++i)
      REQUIRE_THAT(ssol(1, n++), WithinRel(static_cast<Real>(i + j)));
}


TEST_CASE("TestSIM2D.ProjectSolutionMixed")
{
  ...
  for (size_t j = 0; j < 3; ++j)
    for (size_t i = 0; i < 3; ++i)
      REQUIRE_THAT(ssol(1, n++), WithinRel(i/2.0 + j/2.0));
}


TEST_CASE("TestSIM3D.ProjectSolution")
{
  ...
  for (size_t k = 0; k < 2; ++k)
    for (size_t j = 0; j < 2; ++j)
      for (size_t i = 0; i < 2; ++i)
        REQUIRE_THAT(ssol(1, n++), WithinRel(static_cast<Real>(i + j + k)));
}


TEST_CASE("TestSIM3D.ProjectSolutionMixed")
{
  ...
  for (size_t k = 0; k < 3; ++k)
    for (size_t j = 0; j < 3; ++j)
      for (size_t i = 0; i < 3; ++i)
        REQUIRE_THAT(ssol(1, n++), WithinRel(i/2.0 + j/2.0 + k/2.0));
}


TEST_CASE("TestSIM2D.InjectPatchSolution")
{
  ...
  REQUIRE(sim.addMixedMADOF(1, 2));
  sim.injectPatchSolution(sol, lsol, pch, 2, 1);
  for (i = ofs = 0; i < sim.getNoNodes(1); i++, ofs += 2) {
    REQUIRE_THAT(sol[ofs], WithinRel(i+1.0));
    REQUIRE_THAT(sol[ofs+1], WithinRel(i+1.0));
  }
  for (i = 0; i < sim.getNoNodes(2); i++, ofs++)
    REQUIRE_THAT(sol[ofs], WithinAbs(0, 1e-14));

  REQUIRE(sim.addMixedMADOF(2, 2));
  ...
  for (i = ofs = 0; i < sim.getNoNodes(1); i++, ofs++)
    REQUIRE_THAT(sol2[ofs], WithinAbs(0, 1e-14));

  for (i = 0; i < sim.getNoNodes(2); i++, ofs += 2) {
    REQUIRE_THAT(sol2[ofs], WithinRel(i+1.0));
    REQUIRE_THAT(sol2[ofs+1], WithinRel(i+1.0));
  }
}


TEST_CASE("TestSIM2D.ElmConnectivites")
{
  using ConnTest = std::pair<int,ASM::Discretization>;
  const ConnTest param = GENERATE(
    ConnTest{0, ASM::Spline},
    ConnTest{1, ASM::Spline}
#ifdef HAS_LRSPLINE
   ,ConnTest{0, ASM::LRSpline},
    ConnTest{1, ASM::LRSpline}
#endif
  );

  SECTION("Orient " + std::to_string(param.first) +
          (param.second == ASM::Spline ? "Spline" : "LRSpline")) {
    SIM2D sim;
    std::stringstream str;
    sim.opt.discretization = param.second;
    str << "src/ASM/Test/refdata/DomainDecomposition_MPI_2D_4_orient";
    str << param.first << ".xinp";
    ...
    REQUIRE(neighs.size() == 16);
    IntMat r = ref[param.first];
    for (size_t e = 0; e < 16; ++e) {
      if (param.second == ASM::LRSpline) {
      ...
      REQUIRE(neighs[e].size() == r[e].size());
      for (size_t n = 0; n < neighs[e].size(); ++n) {
        REQUIRE(neighs[e][n] == r[e][n]);
      }
    }
  }
}
...
TEST_CASE("TestSIM3D.ElmConnectivities")
{
  using ConnTest = std::pair<int,ASM::Discretization>;
  const ConnTest param = GENERATE(
    ConnTest{0, ASM::Spline},
