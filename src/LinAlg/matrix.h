...
namespace utl //! General utility classes and functions.
{
  ...
  */
  ...
  public:
    ...
    matrix<T>& expandCols(int incRows)
    {
      int newRows = nrow + incRows;
      if (newRows < 1)
      {
        // The matrix size empty
        nrow = 0;
        elem.clear();
      }
      else if (incRows < 0)
	...
	nrow = newRows;
        ...
        for (size_t c = ncol; c > 1; c--, oldMat -= oldRows)
        {
          memcpy(this->ptr(c-1),oldMat,oldRows*sizeof(T));
          for (size_t r = nrow; r > oldRows; r--)
            elem[r-1+nrow*(c-2)] = T(0);
        }
      }
      return *this;
    }
