...
TEST_CASE("TestPETScMatrix.Assemble")
{
  ...
  REQUIRE(mr  == static_cast<int>(v.size()));
  for (int i = 0; i < mr; ++i)
    REQUIRE_THAT(v[i], WithinRel(a[i]));
...
TEST_CASE("TestPETScMatrix.AssembleBasisBlocks")
{
...
TEST_CASE("TestPETScMatrix.AssembleComponentBlocks")
{
  SIM2D sim(2);
...
TEST_CASE("TestPETScMatrix.Copy")
{
  ProcessAdm adm;
  LinSolParams spar;
  ...
  REQUIRE(orig.dim(1) == copy->dim(1));
  REQUIRE(orig.dim(2) == copy->dim(2));
  for (size_t r = 1; r <= orig.dim(1); ++r)
    for (size_t c = 1; c <= orig.dim(2); ++c) {
      PetscScalar a, o;
      MatGetValue(orig.getMatrix(), r-1, c-1, &o);
      MatGetValue(copy->getMatrix(), r-1, c-1, &a);
      REQUIRE_THAT(o, WithinRel(a));
    }
}


TEST_CASE("TestPETScVector.Copy")
{
  ProcessAdm adm;
  adm.dd.setup(5);
  ...
  REQUIRE(orig.dim() == copy1->dim());
  PetscScalar* cv;
  VecGetArray(copy1->getVector(), &cv);
  for (size_t i = 1; i <= orig.dim(); ++i) {
      REQUIRE((*copy1)(i) == orig(i));
      REQUIRE(cv[i-1] == 0.0);
  }
  ...
  REQUIRE(orig.dim() == copy2->dim());
  VecGetArray(copy2->getVector(), &cv);
  for (size_t i = 1; i <= orig.dim(); ++i) {
      REQUIRE((*copy2)(i) == orig(i));
      REQUIRE(cv[i-1] == orig(i));
  }
}


TEST_CASE("TestPETScVectors.Assemble")
{
  SIM2D sim(1);
  sim.read("src/LinAlg/Test/refdata/petsc_test.xinp");
  sim.opt.solver = LinAlg::PETSC;
  ...
  for (int c = 0; c < 2; ++c) {
    ...
    for (PetscInt i = 0; i < static_cast<PetscInt>(v.dim()); ++i) {
      ...
      if (it == mnpc[0].end())
        REQUIRE_THAT(a, WithinAbs(0.0, 1e-14));
      else
        REQUIRE_THAT(a, WithinRel(1.0 + 4*c + std::distance(mnpc[0].begin(), it)));
    }
  }
}
