...
c     reverse communication interface in ARPACK, Buckling mode.
c     This subroutine is based on the test driver DSDRV5 from the
c     symmetric examples of the ARPACK distribution.
c
c     ... Suppose we want to solve K*x = lambda*Kg*x in Buckling mode,
c         where K and Kg are material and geometric stiffness matrices,
c         respectively.
c
...
c
c\Usage:
c  call eig_drv5 ( N, NEV, NCV, SIGMA, D, V, WORK, IERR )
c
c\Arguments
c  N       Integer.  (INPUT)
...
c     eig_sol Linear solver routine that computes inv[K - sigma*Kg]*x
c     eig_av  Matrix vector multiplication routine that computes K*x.
c     eig_mv  Matrix vector multiplication routine that computes Kg*x.
c
c\Author
...
c
      print *, ' '
      print *, ' Entering EIG_DRV5',n,nev,ncv,sigma
c
c     %--------------------------------------------------%
...
c
            ...
            if (ierr .ne. 0) then
               print *, ' '
               print *, ' Error with eig_sol in EIG_DRV5'
               print *, ' '
               return
            end if
c
