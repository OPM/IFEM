      subroutine eig_drv3 (n,nev,ncv,d,v,work,ierr)
c
c $Id: eig_drv3.f,v 1.1 2009-12-08 15:32:49 kmo Exp $
c-----------------------------------------------------------------------
c\BeginDoc
c
c\Name: eig_drv3
...
c     reverse communication interface in ARPACK, inverse mode.
c     This subroutine is based on the test driver DSDRV3 from the
...
c
c\Usage:
c  call eig_drv3 ( N, NEV, NCV, D, V, WORK, IERR )
c
c\Arguments
c  N       Integer.  (INPUT)
...
c     eig_sol Linear solver routine that computes inv[M]*x
...
C
      ...
      Double precision d(ncv,2), v(n,ncv), work(*)
c
c     %--------------%
c     | Local Arrays |
c     %--------------%
...
c
      print *, ' '
      print *, ' Entering EIG_DRV3',n,nev,ncv
c
c     %----------------------------------------------------%
...
c     | different parts of the spectrum.                   |
c     %----------------------------------------------------%
c
      bmat = 'G'
      which = 'LM'
...
c     | iterations allowed.  Mode 2 specified in the      |
c     | documentation of DSAUPD is used (IPARAM(7) = 2).  |
...
c           %--------------------------------------%
c
            call eig_av (work(ipntr(1)), work(ipntr(1)))
            call eig_sol (work(ipntr(1)), work(ipntr(2)), ierr)
            if (ierr .ne. 0) then
               print *, ' '
               print *, ' Error with eig_sol in EIG_DRV3'
               print *, ' '
               return
            end if
c
